...
import { C as CredentialsProviderError, c as chain } from "../smithy__property-provider.mjs";
import { g as getProfileName, p as parseKnownFiles } from "../smithy__shared-ini-file-loader.mjs";
import { f as fromLoginCredentials } from "./credential-provider-login+[...].mjs";
...
const resolveCredentialSource = (credentialSource, profileName, logger) => {
  const sourceProvidersMap = {
    EcsContainer: async (options) => {
      ...
      const { fromContainerMetadata } = await import("../@smithy/credential-provider-imds+[...].mjs");
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is EcsContainer");
      return async () => chain(fromHttp(options ?? {}), fromContainerMetadata(options))().then(setNamedProvider);
    },
    Ec2InstanceMetadata: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Ec2InstanceMetadata");
      const { fromInstanceMetadata } = await import("../@smithy/credential-provider-imds+[...].mjs");
      return async () => fromInstanceMetadata(options)().then(setNamedProvider);
    },
    Environment: async (options) => {
      logger?.debug("@aws-sdk/credential-provider-ini - credential_source is Environment");
      const { fromEnv } = await import("./credential-provider-env+[...].mjs");
...
const resolveAssumeRoleCredentials = async (profileName, profiles, options, callerClientConfig, visitedProfiles = {}, resolveProfileData2) => {
  ...
  if (!options.roleAssumer) {
    const { getDefaultRoleAssumer } = await import("../aws-sdk__nested-clients.mjs").then(function(n) {
      return n.a;
    });
...
const resolveProcessCredentials = async (options, profile) => import("./credential-provider-process+[...].mjs").then(({ fromProcess }) => fromProcess({
  ...options,
  profile
...
const resolveWebIdentityCredentials = async (profile, options, callerClientConfig) => import("./credential-provider-web-identity+[...].mjs").then(({ fromTokenFile }) => fromTokenFile({
