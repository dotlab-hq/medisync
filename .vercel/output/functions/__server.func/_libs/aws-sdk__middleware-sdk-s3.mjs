...
import { e as serdeExports, f as distCjsExports$1 } from "./smithy__core.mjs";
...
import { g as getSmithyContext } from "./smithy__util-middleware.mjs";
import { s as splitStream, h as headStream } from "./smithy__util-stream.mjs";
import { v as validate } from "./aws-sdk__util-arn-parser.mjs";
  ...
  return (next, context) => async (args) => {
    ...
    if (HttpResponse.isInstance(response)) {
      if (response.headers.expires) {
        ...
        try {
          serdeExports.parseRfc7231DateTime(response.headers.expires);
        } catch (e) {
  ...
  return (next, context) => async (args) => {
    if (context.endpointV2) {
      ...
      if (isS3ExpressBucket) {
        distCjsExports.setFeature(context, "S3_EXPRESS_BUCKET", "J");
...
const getS3ExpressHttpSigningPlugin = (config) => ({
  applyToStack: (clientStack) => {
    clientStack.addRelativeTo(s3ExpressHttpSigningMiddleware(config), distCjsExports$1.httpSigningMiddlewareOptions);
  }
});
...
export {
  ...
  SignatureV4S3Express as S,
  getValidateBucketNamePlugin as a,
  getRegionRedirectMiddlewarePlugin as b,
  getS3ExpressPlugin as c,
  getS3ExpressHttpSigningPlugin as d,
  getS3ExpiresMiddlewarePlugin as e,
  getCheckContentLengthHeaderPlugin as f,
