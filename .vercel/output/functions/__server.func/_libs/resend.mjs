...
import { d as distExports } from "./svix.mjs";
var version = "6.9.3";
...
async function render(node) {
  let render2;
  try {
    ({ render: render2 } = await import("../_chunks/render_resend_false.mjs"));
  } catch {
  ...
  async performCreate(payload) {
    if (payload.react) {
      if (!this.renderAsync) try {
        const { renderAsync } = await import("../_chunks/render_resend_false.mjs");
