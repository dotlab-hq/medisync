...
function resolveDirectUrl( raw: string ): string {
    ...
    if ( poolerMatch ) {
        ...
        console.warn( "⚠️   Supabase pooler URL detected — switching to direct connection for cron setup." );
        console.warn( `    Direct URL: ${direct.replace( /:[^:@]+@/, ":***@" )}` );
        console.warn( "    Tip: set DATABASE_URL_DIRECT in .env to skip auto-conversion.\n" );
        return direct;
    }

    // Already a direct / local URL — use as-is
    if ( raw.includes( "localhost" ) || raw.includes( "127.0.0.1" ) || raw.includes( "db." ) ) {
        return raw;
    }
...
async function run() {
    ...
    try {
        ...
        const { rows: allJobs } = await client.query<{ jobname: string; schedule: string; database: string }>(
        ...
        if ( mediJobs.length === 0 ) {
            ...
            console.warn( "    This usually means pg_cron is not enabled on your Supabase project." );
            console.warn( "    Enable it in the Supabase Dashboard → Database → Extensions → pg_cron" );
            console.warn( `    Total jobs in cron.job: ${allJobs.length}\n` );
        } else {
            console.log( "✅  Cron jobs installed successfully:\n" );
            for ( const row of mediJobs ) {
                console.log( `    • ${row.jobname}  (${row.schedule})  db=${row.database}` );
            }
            console.log();
        }
    } finally {
