...
const otpSchema = z.object( {
    code: z.string().length( 6, "OTP must be exactly 6 digits" ).regex( /^\d+$/, "Digits only" ),
} );
...
export function ChangePhoneDialog( {
    ...
    const verifyMut = useMutation( {
        mutationFn: ( data: OtpForm ) =>
            verifyAndUpdatePhone( { data: { newPhone: pendingPhone, code: data.code } } ),
        onSuccess: () => {
            setApiError( "" );
            setStep( "done" );
        ...
        <Dialog open={open} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {step === "enter-phone" && (
                            <>Current: <span className="font-medium">{currentPhone || "Not set"}</span></>
                        )}
                ...
                {step === "enter-phone" && (
                    <Form {...phoneForm}>
                        <form onSubmit={phoneForm.handleSubmit( ( d ) => sendOtpMut.mutate( d ) )} className="space-y-4">
                            <FormField
                ...
                {step === "enter-otp" && (
                    <Form {...otpForm}>
                        <form onSubmit={otpForm.handleSubmit( ( d ) => verifyMut.mutate( d ) )} className="space-y-4">
                            <FormField
                            ...
                            <div className="flex items-center justify-between">
                                <Button
                                    ...
                                    variant="ghost"
                                    size="sm"
                                    onClick={() => { setStep( "enter-phone" ); setApiError( "" ); }}
                                >
                                ...
                                <div className="flex gap-2">
                                    <Button
                                        ...
                                        onClick={() => sendOtpMut.mutate( { newPhone: pendingPhone } )}
                                    >
                                        {sendOtpMut.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Resend OTP"}
                                    </Button>
                ...
                {step === "done" && (
                    <div className="flex flex-col items-center gap-4 py-4">
                        ...
                        <p className="text-center text-sm text-muted-foreground">
                            ...
                            <span className="font-semibold text-foreground">{pendingPhone}</span>.
                        </p>
