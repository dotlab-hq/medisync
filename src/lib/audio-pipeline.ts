  ...
  try {
    const stream = chat( {
      ...
      messages: [
        {
          role: 'system',
          content: 'You analyze emotional tone in speech transcripts. Reply with ONLY valid JSON, no reasoning tags, no markdown.',
        },
  ...
  try {
    const stream = chat( {
      ...
      messages: [
        {
          role: 'system',
          content: 'You format speech transcripts. Keep every word exactly as spoken. Only add punctuation and paragraph breaks. Return ONLY the formatted text.',
        },
        { role: 'user', content: prompt },
      ],
    } )
