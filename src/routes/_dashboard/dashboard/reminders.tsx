...
const TYPE_CONFIG = {
  ...
  other: {
    ...
    badge: 'bg-gray-100 text-gray-700 dark:bg-gray-800/40 dark:text-gray-300',
  },
} as const
...
function RemindersPage() {
  ...
  const ReminderRow = ({ r }: { r: (typeof reminders)[0] }) => {
    ...
    return (
      <div
        className={`flex items-start gap-3 rounded-xl border border-border/60 p-3.5 shadow-sm transition-all ${r.isCompleted ? 'opacity-50' : ''}`}
      >
        <div
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start gap-2 flex-wrap">
            ...
            />
            <span
              className={`text-sm font-medium flex-1 ${r.isCompleted ? 'line-through text-muted-foreground' : ''}`}
            >
              {r.title}
            </span>
            <span
              className={`text-xs px-2 py-0.5 rounded-full font-medium capitalize ${cfg.badge}`}
            >
              {r.type}
            </span>
          </div>
      ...
      <div className="animate-fade-in-up flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-0.5">
            Medications, appointments & checkups
          </p>
        </div>
