...
import { processDueReminders } from "@/server/notify";

export const Route = createFileRoute( "/api/cron/notify-reminders" )( {
    server: {
        handlers: {
            POST: async ( { request } ) => {
                ...
                try {
                    ...
                    console.log( `[cron/notify-reminders] sent=${result.sent} failed=${result.failed}` );
                    return Response.json( { ok: true, ...result } );
                } catch ( err ) {
                    const message = err instanceof Error ? err.message : "Unknown error";
                    console.error( "[cron/notify-reminders] Error:", message );
