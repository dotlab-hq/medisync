            ...
            reminders: {
                ...
                items: {
                    ...
                    required: [
                        ...
                        'time', 'timezone', 'isCompleted', 'createdAt',
                    ],
                    additionalProperties: false,
                },
            },
...
export function createCreateReminderTool( userId: string ) {
    return createReminderDef.server(
        async ( args: any ) => {
            ...
            const [created] = await db
                .insert( reminder )
                .values( { userId, title, description, type, date, time, timezone: tz, toBeSentAt } )
                .returning()
...
export const updateReminderDef = toolDefinition( {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            type: { type: 'string', enum: ['medication', 'appointment', 'checkup', 'other'] },
...
export const deleteReminderDef = toolDefinition( {
    ...
    description: 'Delete a reminder by ID. This is irreversible. Requires user approval.',
    inputSchema: {
        type: 'object',
        properties: {
        ...
        properties: {
            appointments: {
                ...
                items: {
                    ...
                    required: [
                        'id', 'doctorName', 'specialty', 'hospital', 'address',
                        'date', 'time', 'timezone', 'status', 'notes', 'contactNumber', 'createdAt',
                    ],
                    additionalProperties: false,
                },
            },
...
export const createAppointmentDef = toolDefinition( {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            address: { type: 'string', description: 'Address of the appointment (optional)' },
            date: { type: 'string', description: 'Date in YYYY-MM-DD format' },
            time: { type: 'string', description: 'Time in HH:mm (24-hour) format' },
            notes: { type: 'string', description: 'Additional notes (optional)' },
            contactNumber: { type: 'string', description: 'Contact number (optional)' },
            timezone: {
                type: 'string',
                description: 'IANA timezone (optional, defaults to user profile timezone)',
            },
        },
...
export function createCreateAppointmentTool( userId: string ) {
    return createAppointmentDef.server(
        async ( args: any ) => {
            const { doctorName, specialty, hospital, address, date, time, notes, contactNumber, timezone } = args
            let tz = timezone
            if ( !tz ) {
                const u = await db.query.user.findFirst( {
                    where: eq( user.id, userId ),
...
export const deleteAppointmentDef = toolDefinition( {
    ...
    description: 'Delete an appointment by ID. This is irreversible. Requires user approval.',
    inputSchema: {
        type: 'object',
        properties: {
...
export const sendSosEmergencyDef = toolDefinition( {
    ...
    outputSchema: {
        type: 'object',
        properties: {
            sent: { type: 'number', description: 'Number of contacts the SOS was sent to' },
...
export function createSendSosEmergencyTool( userId: string ) {
    return sendSosEmergencyDef.server(
        async ( args: any ) => {
            ...
            let sent = 0
            let failed = 0
            const results: Array<{ name: string; phone: string; status: 'sent' | 'failed' }> = []
