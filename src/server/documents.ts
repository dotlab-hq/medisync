...
function getS3Client() {
    ...
    return new S3Client( {
        ...
        ...( process.env.AWS_S3_ENDPOINT
            ? { endpoint: process.env.AWS_S3_ENDPOINT, forcePathStyle: true, bucketEndpoint: false }
            : {} ),
...
// ── Get user storage ─────────────────────────────────────────────────
export const getUserStorage = createServerFn( { method: "GET" } ).handler( async () => {
...
export const listAllDocuments = createServerFn( { method: "GET" } ).handler( async () => {
    const request = getRequest();
    const session = await auth.api.getSession( { headers: request.headers } );
    if ( !session?.user?.id ) throw new Error( "Unauthorized" );
